      ...
      <nav className="hidden md:flex">
        {navLinks.map((link, index) => (
          <a
            key={link.name}
            href={link.href}
            className={`ml-6 text-[0.9rem] font-medium no-underline transition-colors ${index === 0 ? 'text-accent-orange' : 'text-text-muted hover:text-accent-orange'}`}
          >
            {link.name}
          </a>
        ))}
          ...
          <div className="px-4 py-4 space-y-4 flex flex-col">
            {navLinks.map((link, index) => (
              <a
                ...
                className={`text-[0.9rem] font-medium no-underline transition-colors ${index === 0 ? 'text-accent-orange' : 'text-text-muted hover:text-accent-orange'}`}
              >
                {link.name}
              </a>
            ))}
